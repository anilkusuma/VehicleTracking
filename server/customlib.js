...
module.exports.validateCookies = function(req,callback){
    ...
    if(req.query.validator != undefined && req.query.selector != undefined && req.query.userId != undefined) {
        ...
        console.log("Values are " + selector + " " + validator + " " + userId);

        app.models.Login.findById(userId, {include: ['user', 'account']}, function (err, user) {
            console.log("Values are " + err + " " + user);
            if (err) {
        ...
        let selector = cookies.selector;
        let validator = cookies.validator;
