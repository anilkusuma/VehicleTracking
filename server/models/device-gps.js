module.exports = function(DeviceGps) {
    ...
    DeviceGps.lastTwoPacketsOfVehicles = function(req,res,next){
		...
		customLib.validateCookies(req,function(status,user){
			if(status){
				DeviceGps.find({where:{'deviceImei':req.query.imei},order:'packetTime DESC',limit:2},function(err,instance){
					if(err){
                        result = {};
    ...
    DeviceGps.replayPackets = function(req,res,next){
		...
		customLib.validateCookies(req,function(status,user){
			if(status){
				DeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:req.query.startTime}},{'packetTime':{lte:req.query.endTime}}]},order:'packetTime DESC'},function(err,instance){
					if(err){
						result = {};
						result.returnStatus = "ERROR";
						res.send(result);
    ...
    DeviceGps.overSpeedReport = function(req,res,next){
        ...
        customLib.validateCookies(req,function(status,user){
            ...
            if(status){
                DeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:startTime}},{'packetTime':{lte:endTime}}]},order:'packetTime DESC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    DeviceGps.stoppageReport = function(req,res,next){
        ...
        customLib.validateCookies(req,function(status,user){
            ...
            if(status){
                DeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:startTime}},{'packetTime':{lte:endTime}}]},order:'packetTime ASC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    DeviceGps.processVehicleAndFence = function(vehicles,fences,startTime,endTime,timeInterval,userId,companyId,callback){
        ...
        (function processOne(){
            if(i>=vehicles.length){
                ...
                DeviceGps.find({where:{and:[{'deviceImei':deviceImei},{'packetTime':{gte:startTime}},{'packetTime':{lte:endTime}},{'userId':userId},{'companyId':companyId}]},order:'packetTime ASC'},function(err,instance){
                    if(err){
                        i++
    ...
    DeviceGps.distanceReport = function(req,res,next){
        ...
        customLib.validateCookies(req,function(status,user){
            if(status){
                ...
                DeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:req.query.startTime}},{'packetTime':{lte:req.query.endTime}},{'userId':userId},{'companyId':companyId}]},order:'packetTime DESC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
