module.exports = function(TestDeviceGps) {
	var customLib = require('../../server/customlib.js');
    var app = require('../../server/server.js');
    var moment = require('moment-timezone');
    var bodyParser = require('body-parser').urlencoded({extended: true});
    TestDeviceGps.lastTwoPacketsOfVehicles = function(req,res,next){
        var result = {};
		customLib.validateCookies(req,function(status,user){
            var todayEnd = moment().tz('Asia/Calcutta').format('YYYY-MM-DD HH:mm:ss');
			if(status){
				TestDeviceGps.find({where:{'and':[{'deviceImei':req.query.imei},{'packetTime':{lte:todayEnd}}]},order:'packetTime DESC',limit:2},function(err,instance){
					if(err){
                        result = {};
    ...
    TestDeviceGps.replayPackets = function(req,res,next){
        var result = {};
        result.responseData = {};
        var start_time = null;
		var end_time = null;
		...
		customLib.validateCookies(req,function(status,user){
			if(status){
				TestDeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:req.query.startTime}},{'packetTime':{lte:todayEnd}},{'packetTime':{lte:req.query.endTime}}]},order:'packetTime DESC'},function(err,instance){
					if(err){
						result = {};
						result.returnStatus = "ERROR";
						res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'replayPackets',
        {
            ...
            http:{path:'/testgps/GetReplay',verb:'get'}
        }
    );  

    TestDeviceGps.getTodayPackets = function(req,res,next){
        var result = {};
        result.responseData={};
        ...
        customLib.validateCookies(req,function(status,user){
            ...
            var todayEnd = moment().tz('Asia/Calcutta').format('YYYY-MM-DD HH:mm:ss');
            if(status){
                TestDeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:todayStart}},{'packetTime':{lte:todayEnd}}]},order:'packetTime DESC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'getTodayPackets',
        {
            ...
            http:{path:'/testgps/GetTodayPackets',verb:'get'}
        }
    );
    TestDeviceGps.getTodaysOdometer = function(req,res,next){
        var result = {};
        customLib.validateCookies(req,function(status,user){
            var today = moment().tz('Asia/Calcutta').format('YYYY-MM-DD');
            var todayStart = today+' 00:00:00';
    ...
    TestDeviceGps.remoteMethod(
        'getTodaysOdometer',
        {
            ...
            http:{path:'/testgps/TodaysOdometer',verb:'get'}
        }
    );
     TestDeviceGps.settingPackets = function(req,res,next){
        var result = {};
        customLib.validateCookies(req,function(status,user){
            ...
            if(status){
                TestDeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:selStartDate}},{'packetTime':{lte:selEndDate}}]},order:'packetTime DESC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'settingPackets',
        {
            ...
            http:{path:'/testgps/SettingPackets',verb:'get'}
        }
    );

    TestDeviceGps.detailedDayReport = function(req,res,next){
        var result = {};
        customLib.validateCookies(req,function(status,user){
            var selDate = moment(req.query.selDate,'YYYY-MM-DD').format('YYYY-MM-DD');
            var selStartDate = selDate+' 00:00:00';
            ...
            var todayEnd = moment().tz('Asia/Calcutta').format('YYYY-MM-DD HH:mm:ss');
            if(status){
                TestDeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:selStartDate}},{'packetTime':{lte:todayEnd}},{'packetTime':{lte:selEndDate}}]},order:'packetTime DESC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'detailedDayReport',
        {
            ...
            http:{path:'/testgps/DetailedReport',verb:'get'}
        }
    );
    TestDeviceGps.overSpeedReport = function(req,res,next){
        var result = {};
        customLib.validateCookies(req,function(status,user){
            ...
            var todayEnd = moment().tz('Asia/Calcutta').format('YYYY-MM-DD HH:mm:ss');
            if(status){
                TestDeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:startTime}},{'packetTime':{lte:todayEnd}},{'packetTime':{lte:endTime}}]},order:'packetTime DESC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'overSpeedReport',
        {
            ...
            http:{path:'/testgps/SpeedReport',verb:'get'}
        }
    );


    TestDeviceGps.stoppageReport = function(req,res,next){
        var result = {};
        customLib.validateCookies(req,function(status,user){
            var startTime = moment(req.query.startTime,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss');
            var endTime = moment(req.query.endTime,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss');
            ...
            var todayEnd = moment().tz('Asia/Calcutta').format('YYYY-MM-DD HH:mm:ss');
            if(status){
                TestDeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:startTime}},{'packetTime':{lte:todayEnd}},{'packetTime':{lte:endTime}}]},order:'packetTime ASC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'stoppageReport',
        {
            ...
            http:{path:'/testgps/StoppageReport',verb:'get'}
        }
    );

    TestDeviceGps.fenceReport = function(req,res,next){
        var result = {};
        customLib.validateCookies(req,function(status,user){
            if(status){
                                    ...
                                    if(object.fences[0].geoFenceId == -1){
                                        app.models.VtsGeoFencing.find({'where':{'and':[{'userId':userId},{'companyId':companyId}]}},function(err,instance){
                                            if(err){
                                                ...
                                                TestDeviceGps.processVehicleAndFence(object.vehicles,object.fences,object.startTime,object.endTime,object.timeInterval,userId,companyId,function(result){
                                                    res.send(result);
                                                    return;
                                                });
                                            }
    ...
    TestDeviceGps.remoteMethod(
        'fenceReport',
        {
            ...
            http:{path:'/testgps/FenceReport',verb:'post'}
        }
    );

    TestDeviceGps.processVehicleAndFence = function(vehicles,fences,startTime,endTime,timeInterval,userId,companyId,callback){
        var i=0;
        var result = {};
        result.responseData = [];
        result.returnStatus = "SUCCESS";
        ...
        (function processOne(){
            if(i>=vehicles.length){
                ...
                TestDeviceGps.find({where:{and:[{'deviceImei':deviceImei},{'packetTime':{gte:startTime}},{'packetTime':{lte:todayEnd}},{'packetTime':{lte:endTime}},{'userId':userId},{'companyId':companyId}]},order:'packetTime ASC'},function(err,instance){
                    if(err){
                        i++
    ...
    TestDeviceGps.distanceReport = function(req,res,next){
        var result = {};
        result.responseData = {};
        var start_time = null;
        var end_time = null;
        ...
        customLib.validateCookies(req,function(status,user){
            if(status){
                ...
                TestDeviceGps.find({where:{and:[{'deviceImei':req.query.imei},{'packetTime':{gte:req.query.startTime}},{'packetTime':{lte:todayEnd}},{'packetTime':{lte:req.query.endTime}},{'userId':userId},{'companyId':companyId}]},order:'packetTime DESC'},function(err,instance){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'distanceReport',
        {
            ...
            http:{path:'/testgps/DistanceReport',verb:'get'}
        }
    ); 


    TestDeviceGps.createPacket = function(req,res,next){
        customLib.validateCookies(req,function(validated,user){
            if(validated){
                ...
                TestDeviceGps.create(packet,function(err,obj){
                    if(err){
                        result = {};
    ...
    TestDeviceGps.remoteMethod(
        'createPacket',
        {
            ...
            http:{path:'/testgps/CreatePacket',verb:'post'}
        }
    ); 

    TestDeviceGps.updatePacket = function(req,res,next){
        customLib.validateCookies(req,function(validated,user){
            if(validated){
                var user = JSON.parse(user);
                var userType = user.vtsLogin.userType;
                ...
                TestDeviceGps.findById(req.body.packetId,function(err,instance){
                    if(err){
                        result = {};
                        console.log('Error is '+err);
                        result.returnStatus="ERROR";
    ...
    TestDeviceGps.remoteMethod(
        'updatePacket',
        {
            ...
            http:{path:'/testgps/UpdatePacket',verb:'post'}
        }
    );

    TestDeviceGps.deletePacket = function(req,res,next){
        var result = {};
        customLib.validateCookies(req,function(validated,user){
            if(validated){
                ...
                TestDeviceGps.destroyAll({'packetId':req.query.packetId,'companyId': companyId},function(err){
                    if(err){
                        result = {};
                        result.returnStatus = "ERROR";
                        res.send(result);
    ...
    TestDeviceGps.remoteMethod(
        'deletePacket',
        {
            ...
            http:{path:'/testgps/deletePacket',verb:'get'}
        }
    );
