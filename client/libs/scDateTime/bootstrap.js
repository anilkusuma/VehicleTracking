        ...
        link: function(scope,
            ...
            $popover(element, {
                ...
                controller: [
                    '$scope', function ($scope) {
                        // See https://github.com/SimeonC/sc-date-time#options for details
                        var passthroughAttrs = ['displayMode', 'defaultDate', 'defaultMode', 'mindate', 'maxdate','parentModelValue','ngChange']
                        angular.forEach(passthroughAttrs, function (key) {
                            $scope[key] = attrs[key];
                        });
                        if(angular.isDate(ngModelCtrl.$modelValue)) $scope.model = ngModelCtrl.$modelValue;
                        ...
                        $scope.$watch('model', function(value){
                            ...
                            if($scope.parentModelValue != undefined && $scope.parentModelValue != null)
                                $scope.parentModelValue = formatDate(value);
                        }, true);
                    }
                ]
            });
